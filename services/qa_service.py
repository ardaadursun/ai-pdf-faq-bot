...
class QAService:
    ...
    def find_relevant_chunks(self, question: str, pdf_id: int = None, top_k: int = 3) -> List[dict]:
...
Antworte präzise und kurz basierend auf dem Kontext. Wenn die Antwort nicht im Kontext steht, antworte mit "Nicht im Dokument enthalten".
Antworte nur mit der direkten Antwort, ohne zusätzliche Erklärungen."""

            # Call OpenAI API
            response = self.openai_client.chat.completions.create(
                model=config.OPENAI_MODEL,
                messages=[
                    {"role": "system", "content": "Du bist ein hilfreicher Assistent, der Fragen zu Dokumenten präzise beantwortet."},
